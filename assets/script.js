...
var init = function () {
  ...
  //create a loop that loops through stored cities and calls render last searched function for each

  for (let i = 0; i < storedCities.length; i++) {
    const city = storedCities[i];

    renderPastSearch(city);
  }
...
searchMainBtnEl.addEventListener("click", function () {
  ...
  fetch(
    "https://api.geoapify.com/v1/geocode/search?text=" +
      city +
      "&apiKey=51181722fe3645919bd3cf77a3c846e9",
  ...
  fetch(
    ...
    .then((data) => {
      ...
      cityTempEl.innerHTML =
        "<strong>Current Tempt:</strong> " + cityTemp + "° fahrenheit";
...
function renderPastSearch(city) {
  var newSearchBtn = document.createElement("button");
  newSearchBtn.innerHTML = city;
